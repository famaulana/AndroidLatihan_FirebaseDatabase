...
class UploadFileStorage : AppCompatActivity(){
        ...
        image_placeholder.setOnClickListener {
            when{
                (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)->{
                    if (ContextCompat.checkSelfPermission(this@UploadFileStorage, Manifest.permission.READ_EXTERNAL_STORAGE)!= PackageManager.PERMISSION_GRANTED) {
        ...
        when(requestCode){
            PERMISSION_REQUEST_CODE ->{
                if (grantResults.isEmpty() || grantResults[0] == PackageManager.PERMISSION_DENIED){
                    Toast.makeText(this@UploadFileStorage, "Izin Ditolak!!", Toast.LENGTH_SHORT).show()
                }else{
                    imageChooser()
                }
            }
        ...
        when(requestCode){
            REQUEST_IMAGE ->{
                ...
                try {
                    ...
                    Glide.with(this).load(bitmap).override(250,250).centerCrop().into(image_placeholder)
                }catch (x : IOException){
                    x.printStackTrace()
                }
            }
    ...
    fun uploadDatas(){
        ...
        ref.putFile(filePathImage).addOnSuccessListener {
            Toast.makeText(this@UploadFileStorage, "Success Upload", Toast.LENGTH_SHORT).show()
